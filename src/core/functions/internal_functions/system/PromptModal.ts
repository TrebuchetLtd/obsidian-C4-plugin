import { JudgerError } from "../../../../utils/Error";
import {
  ButtonComponent,
  Modal,
  Platform,
...
export class PromptModal extends Modal {
  ...
  private reject: (reason?: JudgerError) => void;
  ...
  onClose(): void {
    ...
    if (!this.submitted) {
      // TOFIX: for some reason throwing JudgerError on iOS causes the app to freeze.
      // this.reject(new JudgerError("Cancelled prompt"));
      this.reject();
    }
  }
  ...
  async openAndGetValue(
    ...
    reject: (reason?: JudgerError) => void
  ): Promise<void> {
